...
#define sc301iot_SUPPORT_30FPS_SCLK (102000000)
...
static int sc301iot_set_expo(struct tx_isp_subdev *sd, int value)
{
	...
	ret += sc301iot_write(sd, 0x3e09, (unsigned char)(again & 0xff));
	ret += sc301iot_write(sd, 0x3e08, (unsigned char)(((again >> 8) & 0xff)));
	if (ret < 0)
		return ret;
...
static int sc301iot_set_logic(struct tx_isp_subdev *sd, int value)
{
	...
	ret = sc301iot_read(sd, 0x3974, &temp_val);
	if(ret != 0){
		return ret;
	...
	if((temp_val >= 0x40) && (cur_lut_node >= 65)) {
		sc301iot_attr.max_again = lut[cur_lut_node - 2].gain;
		cur_lut_node -= 2;
		node_change = 1;
...
static int sc301iot_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	case TX_SENSOR_MAX_FPS_100:
		...
		sc301iot_attr.max_integration_time = 0x320 - 5;
		memcpy((void*)(&(sc301iot_attr.mipi)),(void*)(&sc301iot_mipi),sizeof(sc301iot_mipi));
		break;
	default:
		ISP_ERROR("do not support max framerate %d in mipi mode\n",sensor_max_fps);
